...
export function AdminSettingsSection({ showToast }: AdminSettingsSectionProps) {
  ...
  return (
    <section className="grid w-full gap-6">
      ...
      <section className="rounded-2xl border-2 border-[var(--line)] bg-[var(--white)] p-8">
        <div className="mb-6">
        ...
        <div className="rounded-xl border border-[var(--line)] bg-[var(--surface)] p-6">
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[var(--line)] pb-5">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${soundAlertsEnabled ? "bg-[var(--success-bg)] text-[var(--success-text)]" : "bg-[var(--neutral-track)] text-[var(--muted)]"}`}>{soundAlertsEnabled ? "Active" : "Inactive"}</span>
              </div>
            ...
            <button
              ...
              className={`relative h-9 w-16 rounded-xl border p-1 transition ${soundAlertsEnabled ? "border-[var(--success-text)] bg-[var(--success-bg)]" : "border-[var(--line)] bg-[var(--neutral-track)]"}`}
              onClick={() => {
              ...
              type="button"
            >
              <span className={`block h-7 w-7 rounded-full bg-[var(--white)] transition ${soundAlertsEnabled ? "translate-x-7 bg-[var(--success-text)]" : "translate-x-0"}`} />
            </button>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--line)] py-5">
            <div>
              <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-[var(--brand-navy)]">
                ...
                <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs ${desktopNotificationsAllowed ? "bg-[var(--success-bg)] text-[var(--success-text)]" : "bg-[var(--warning-bg)] text-[var(--warning-text)]"}`}>
              ...
              <p className="mt-2 text-sm text-[var(--warning-text)]">Click the button to enable desktop notifications for inactive tabs</p>
            </div>

            <button className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-[var(--brand-orange)] bg-[var(--brand-orange)] px-4 text-sm font-semibold text-[var(--brand-orange-contrast)] hover:bg-[var(--brand-orange-hover)]" onClick={allowDesktopNotifications} type="button">
          ...
          <div className="pt-5">
            <button className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-[var(--brand-orange)] bg-[var(--brand-orange)] px-4 text-sm font-semibold text-[var(--brand-orange-contrast)] hover:bg-[var(--brand-orange-hover)]" onClick={testSoundNotification} type="button">
        ...
        <div className="mt-7 rounded-xl bg-[var(--surface)] p-6 text-sm text-[var(--muted)]">
