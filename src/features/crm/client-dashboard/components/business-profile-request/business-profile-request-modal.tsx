...
} from "@/features/crm/client-dashboard/business-profile-request-format";
import type { BusinessProfileRequest } from "@/features/crm/client-dashboard/business-profile-request-storage";
import {
  businessProfileInputClassName,
  businessProfileLabelClassName,
} from "@/features/crm/client-dashboard/components/business-profile-request/business-profile-form-styles";
    ...
    <div className="border-t border-[var(--line)] pt-6">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        {documents.map((document) => (
          <div className="relative h-28 w-28 overflow-hidden rounded-xl border border-[var(--line)] bg-[var(--surface)]" key={document.id}>
            <div className="grid h-full place-content-center bg-[linear-gradient(135deg,#f8fafc,#e2e8f0)] p-2 text-center">
              <span className="body-xsm-regular line-clamp-3 subtext">{document.label}</span>
            </div>
            <button
        ...
        <button
          ...
          type="button"
        >
          <Upload aria-hidden="true" className="mx-auto text-[var(--muted)]" size={24} strokeWidth={1.6} />
      ...
      <p className="body-xsm-regular m-0 mt-3 subtext">Supported format: JPG, PNG, PDF (Max 5MB each)</p>
    </div>
  );
}

export function BusinessProfileRequestModal({ mode, onClose, onSave, request }: BusinessProfileRequestModalProps) {
        ...
        <div className="border-b border-[var(--line)] px-6 py-5">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-[var(--brand-orange-soft)] px-3 py-1 text-xs font-medium text-[var(--color-adsfixter-primary)]">
              {draft.status === "Pending" ? <Loader2 aria-hidden="true" className="animate-spin" size={12} strokeWidth={2} /> : null}
              {getBusinessProfileStatusLabel(draft.status)}
            </span>
          </div>
          <p className="body-sm-regular m-0 mt-2 subtext">Submitted on {formatBusinessProfileSubmittedDateTime(draft.submittedAt)}</p>
        </div>
        ...
        <div className="px-6 py-5">
          {isViewMode ? (
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <DetailField label="Business Phone" value={`+880 ${draft.businessPhone}`} />
            ...
            >
              <div className="grid gap-4 md:grid-cols-2">
                <FormField label="Business Name">
                  <input className={businessProfileInputClassName} onChange={(event) => updateField("businessName", event.target.value)} value={draft.businessName} />
                </FormField>
                <FormField label="Business Email">
                  <input className={businessProfileInputClassName} onChange={(event) => updateField("businessEmail", event.target.value)} type="email" value={draft.businessEmail} />
                </FormField>
                ...
                <FormField label="Business Address">
                  <input className={businessProfileInputClassName} onChange={(event) => updateField("businessAddress", event.target.value)} value={draft.businessAddress} />
                </FormField>
                <FormField label="Website Link">
                  <div className="relative">
                    ...
                    <input className={`${businessProfileInputClassName} pl-9`} onChange={(event) => updateField("websiteLink", event.target.value)} value={draft.websiteLink} />
                  </div>
                </FormField>
                <FormField label="Facebook Page Link">
                  <div className="relative">
                    ...
                    <input className={`${businessProfileInputClassName} pl-9`} onChange={(event) => updateField("facebookLink", event.target.value)} value={draft.facebookLink} />
                  </div>
                </FormField>
                ...
                <FormField label="Designation of User">
                  <select className={businessProfileInputClassName} onChange={(event) => updateField("designation", event.target.value)} value={draft.designation}>
