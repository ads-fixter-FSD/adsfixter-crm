import { blackCategoryItems, grayCategoryItems, whiteCategoryItems } from "@/features/crm/client-dashboard/components/business-profile-request/business-profile-form-styles";
...
function CategoryCard({ items, title, twoColumns = false }: CategoryCardProps) {
  return (
    <div className="rounded-xl border border-[var(--line)] bg-[var(--white)] p-4">
      <h3 className="body-sm-medium m-0 border-b border-[var(--line)] pb-3 primary-text">{title}</h3>
      <ul className={`body-xsm-regular m-0 mt-3 grid list-none gap-2 p-0 subtext ${twoColumns ? "md:grid-cols-2" : ""}`}>
        {items.map((item) => (
          <li className="flex items-start gap-2" key={item}>
            <span aria-hidden="true" className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--muted)]" />
            <span>{item}</span>
          </li>
        ))}
      </ul>
...
export function ProductCategoryGuide({ variant = "card" }: ProductCategoryGuideProps) {
  const containerClassName = variant === "card" ? "rounded-xl border border-[var(--line)] bg-[var(--white)] p-5 max-[720px]:p-4" : "mt-6 rounded-xl border border-[var(--line)] bg-[var(--surface)] p-4";

  return (
    <div className={containerClassName}>
      <h2 className="h6-medium m-0 primary-text">Product Category Guide</h2>
