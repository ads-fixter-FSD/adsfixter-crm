...
import type { AccessSharingMethod, AdAccountRequest } from "@/features/crm/client-dashboard/sections/ad-account-request/ad-account-request-storage";
...
export function AdAccountRequestFormBlock({
  ...
  const updateValue = <K extends keyof AdAccountFormValues>(field: K, value: AdAccountFormValues[K]) => {
    onChange({ ...values, [field]: value });
  };

  return (
    <div className="grid gap-6 rounded-xl border border-[var(--line)] bg-[var(--white)] p-5 max-[720px]:p-4">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--line)] pb-5">
        <div>
          <h2 className="body-sm-medium m-0 primary-text">Ads Details</h2>
          <p className="body-xsm-regular m-0 mt-1 subtext">Tell us about the products or services you want to advertise.</p>
        </div>

        {onContactSupport ? (
          <SecondaryButton className="min-h-10 gap-2 bg-[var(--brand-navy)] px-4 text-[var(--white)] hover:bg-[var(--black)]" onClick={onContactSupport} type="button">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <BusinessProfileFormField htmlFor={`business-profile-${blockId}`} label="Select Business Profile" required>
          <select
            className={businessProfileInputClassName}
            id={`business-profile-${blockId}`}
            onChange={(event) => updateValue("businessProfileId", event.target.value)}
        ...
        <BusinessProfileFormField htmlFor={`ad-account-name-${blockId}`} label="Ad Account Name" required>
          <input
            className={businessProfileInputClassName}
            id={`ad-account-name-${blockId}`}
            onChange={(event) => updateValue("adAccountName", event.target.value)}
      ...
      <BusinessProfileFormField htmlFor={`advertising-products-${blockId}`} label="Advertising Product Details" required>
        <ProductTagsInput
          id={`advertising-products-${blockId}`}
          onChange={(products) => updateValue("advertisingProducts", products)}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <BusinessProfileFormField htmlFor={`time-zone-${blockId}`} label="Time Zone" required>
          <select
            className={businessProfileInputClassName}
        ...
        <BusinessProfileFormField htmlFor={`monthly-spend-${blockId}`} label="Expected Monthly Spend (USD)" required>
          <div className="relative">
            <span className="body-sm-regular pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 subtext">$</span>
            <input
              className={`${businessProfileInputClassName} pl-7`}
              id={`monthly-spend-${blockId}`}
              inputMode="decimal"
              onChange={(event) => updateValue("expectedMonthlySpend", event.target.value)}
              placeholder="e.g. 100"
      ...
      <div className="grid gap-4">
        <div>
          <h3 className="body-sm-medium m-0 primary-text">Access / Sharing Method</h3>
          <p className="body-xsm-regular m-0 mt-1 subtext">You can add multiple IDs or emails separated by commas.</p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {accessSharingOptions.map((option) => {
            ...
            return (
              <label
                className={`body-sm-regular flex cursor-pointer items-center gap-2 rounded-xl border px-4 py-3 ${
                  isSelected ? "border-[var(--color-adsfixter-primary)] bg-[var(--brand-orange-soft)]" : "border-[var(--line)] bg-[var(--white)]"
                }`}
                key={option.id}
              >
                <input
        ...
        <textarea
          className="min-h-28 w-full rounded-lg border border-[var(--line)] bg-[var(--field-bg)] px-3 py-3 text-sm text-[var(--brand-navy)] outline-none focus:border-[var(--color-adsfixter-primary)]"
          onChange={(event) => updateValue("accessSharingDetails", event.target.value)}
          placeholder="Enter Business Manager ID(s) or email address(es)"
          value={values.accessSharingDetails}
        />
        <p className="body-xsm-regular m-0 subtext">e.g. 123456789912, 9875247148504</p>
      </div>

      <div className="flex gap-2 rounded-lg bg-[var(--brand-orange-soft)] px-4 py-3">
        <Info aria-hidden="true" className="mt-0.5 shrink-0 text-[var(--color-adsfixter-primary)]" size={16} strokeWidth={1.8} />
        <p className="body-xsm-regular m-0 text-[var(--brand-navy)]">
          <strong>Important Note:</strong> Please make sure all information is accurate. Wrong or incomplete information may delay the approval process.
        </p>
      </div>
    </div>
  );
...
export function createFormValuesFromRequest(request: AdAccountRequest): AdAccountFormValues {
  return {
...
export function createEmptyAdAccountFormValues(businessProfileId = ""): AdAccountFormValues {
  return {
    businessProfileId,
    adAccountName: "",
    advertisingProducts: [],
